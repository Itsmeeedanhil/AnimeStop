...
export function isBot(userAgent, clientSignals = {}) {
  ...
  // 4. Check for automated headless browser signatures
  if (lower.includes('headlesschrome') || lower.includes('phantomjs') || lower.includes('puppeteer')) {
    return true;
  }
...
export function detectDevice(userAgent) {
  ...
  const lower = userAgent.toLowerCase();
  if (lower.includes('tauri') || lower.includes('windowsapp') || lower.includes('electron') || lower.includes('animestop-desktop')) {
    return 'Windows App';
  }
