...
export const saveLocalLibrary = (data) => {
  ...
  try {
    localStorage.setItem('animestop_local_library', JSON.stringify({
...
export const LibraryApi = {
  getLibrary: async () => {
    ...
    try {
      ...
      if (data && Array.isArray(data.watchlist)) {
        // Merge local data into database response so nothing is ever lost
        ...
        local.history.forEach(lh => {
          if (!mergedHistory.some(h => Number(h.anime_id) === Number(lh.anime_id) && Number(h.episode_number) === Number(lh.episode_number))) {
  ...
  toggleWatchlist: async (anime) => {
    ...
    // 2. Dispatch custom window event so all open tabs and components react instantly
    if (typeof window !== 'undefined') {
