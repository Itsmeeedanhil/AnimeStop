...
function extractKitsuEnglishTitle(attr) {
  if (!attr) return null;
  if (attr.titles?.en && !hasCyrillic(attr.titles.en)) return attr.titles.en;
  if (attr.titles?.en_us && !hasCyrillic(attr.titles.en_us)) return attr.titles.en_us;

  if (Array.isArray(attr.abbreviatedTitles)) {
    const englishAbbr = attr.abbreviatedTitles.find(
      (t) =>
        typeof t === 'string' &&
        !/[\u0400-\u04FF\u3040-\u30ff\u3400-\u4dbf\u4e00-\u9fff\uf900-\ufaff\uff66-\uff9f\uac00-\ud7af]/.test(t)
    );
    if (englishAbbr) return englishAbbr;
  }

  if (attr.canonicalTitle && !hasCyrillic(attr.canonicalTitle)) {
    return attr.canonicalTitle;
  }
  return null;
}
  ...
  if (malId) {
    try {
      ...
      if (res.ok) {
        ...
        const kitsuTitle = extractKitsuEnglishTitle(item?.attributes);
...
function mapShikimoriAnime(item) {
  ...
  return {
    ...
    episodesAired: item.episodesAired !== undefined && item.episodesAired !== null ? item.episodesAired : (item.status === 'ongoing' ? (item.episodesAired || 0) : (item.episodes || 12)),
    ...
    trailer,
    nextAiringEpisode: item.nextEpisodeAt ? {
      episode: (item.episodesAired || 0) + 1,
      airingAt: Math.floor(new Date(item.nextEpisodeAt).getTime() / 1000),
    } : null,
  };
}
...
export async function getShikimoriAnimeDetails(id) {
  ...
  if (hasCyrillic(item.description) || !item.description) {
    try {
      ...
      if (item.english && !hasCyrillic(item.english)) {
        mapped.title.english = item.english;
      } else if (englishMeta?.englishTitle && !hasCyrillic(englishMeta.englishTitle)) {
...
export async function searchShikimoriAnime({
  ...
  const limit = Math.min(parseInt(per_page, 10) || 24, 50);
  const pageNum = Math.max(1, parseInt(page, 10) || 1);
  const order = orderMap[sort] || 'popularity';

  const query = `
    query SearchShikimori(
      $search: String
      $page: Int
      $limit: Int
      $order: OrderEnum
  ...
  if (genre && genre !== 'All' && genre !== '') {
    genreId = SHIKIMORI_GENRE_MAP[genre] || (String(parseInt(genre, 10)) === genre ? genre : undefined);
  }

  let kindParam = undefined;
  if (format && format !== 'All' && format !== '') {
    kindParam = SHIKIMORI_KIND_MAP[format.toUpperCase()] || format.toLowerCase();
  }

  let statusParam = undefined;
  if (status && status !== 'All' && status !== '') {
    const sLower = status.toLowerCase();
    if (sLower === 'ongoing' || sLower === 'releasing' || sLower === 'airing') {
      statusParam = 'ongoing';
    } else if (sLower === 'completed' || sLower === 'finished' || sLower === 'released') {
      statusParam = 'released';
    } else if (sLower === 'latest' || sLower === 'upcoming') {
      statusParam = 'latest';
    }
  }

  let seasonParam = undefined;
  if (season && season !== 'All' && season !== '') {
    const s = season.toLowerCase();
    if (year) {
      seasonParam = `${year}_${s}`;
    } else {
      seasonParam = s;
    }
  }
  ...
  const estimatedTotal = hasNextPage ? Math.max((pageNum + 5) * limit, 240) : (pageNum - 1) * limit + list.length;
  const estimatedLastPage = hasNextPage ? Math.max(pageNum + 5, 10) : pageNum;
