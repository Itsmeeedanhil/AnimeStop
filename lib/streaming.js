export function resolveStreamingServers(anilistId, episodeNumber = 1, malId = null) {
  ...
  const animoParams = 'k=1&autoPlay=1';
  ...
  const serversList = [
    {
      id: '4animo-auto',
      name: '4Animo (Auto)',
      url: `${embedBaseUrl}/ani/${anilistId}/${episodeNumber}/sub?${animoParams}`,
    },
    {
      id: 'cinesrc',
      name: '4Animo (HD-1)',
      url: `${embedBaseUrl}/hd-1/ani/${anilistId}/${episodeNumber}/sub?${animoParams}`,
    },
    {
      id: 'vidcore',
      name: '4Animo (HD-2)',
      url: `${embedBaseUrl}/hd-2/ani/${anilistId}/${episodeNumber}/sub?${animoParams}`,
    },
    {
      id: 'zoryva',
      name: '4Animo MAL (HD-1)',
      url: `${embedBaseUrl}/hd-1/mal/${targetMalId}/${episodeNumber}/sub?${animoParams}`,
    },
    {
      id: '4animo-mal-hd2',
      name: '4Animo MAL (HD-2)',
      url: `${embedBaseUrl}/hd-2/mal/${targetMalId}/${episodeNumber}/sub?${animoParams}`,
    },
    {
      id: 'vidsrc',
      name: 'VidSrc',
...
export function getStreamData(animeId, episode = 1, animeDetails = null) {
  ...
  let releasedEpisodesCount = 12;

  if (status === 'NOT_YET_RELEASED') {
    releasedEpisodesCount = totalEpisodes > 0 ? totalEpisodes : Math.max(12, episode);
  } else if (status === 'RELEASING') {
    if (nextAiring?.episode && nextAiring.episode > 1) {
      releasedEpisodesCount = Math.max(episode, parseInt(nextAiring.episode, 10) - 1);
    } else if (totalEpisodes) {
      releasedEpisodesCount = parseInt(totalEpisodes, 10);
    } else {
      releasedEpisodesCount = Math.max(12, episode);
    }
  } else {
    releasedEpisodesCount = parseInt(totalEpisodes || 12, 10);
  }

  const isUnreleased = false;
