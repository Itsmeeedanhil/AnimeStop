export function resolveStreamingServers(anilistId, episodeNumber = 1, malId = null) {
  ...
  // Hardsub parameters for universal Chrome/Brave/Edge/Mobile pixel rendering
  const hardsubParams = 'k=1&autoPlay=1&sub=en&sub_type=hard';
  const softsubParams = 'k=1&autoPlay=1&sub=en';

  const serversList = [
    {
      id: '4animo-hd1',
      name: 'Server 1 (HiAnime HD - Hardsub)',
      badge: 'Universal Subs (Chrome/Brave/Edge)',
      url: `${embedBaseUrl}/hd-1/ani/${anilistId}/${episodeNumber}/sub?${hardsubParams}`,
    },
    {
      id: '4animo-hd2',
      name: 'Server 2 (HiAnime HD-2 - Hardsub)',
      badge: 'HiAnime Hardsub',
      url: `${embedBaseUrl}/hd-2/ani/${anilistId}/${episodeNumber}/sub?${hardsubParams}`,
    },
    {
      id: '4animo-ani',
      name: 'Server 3 (4Animo Direct - Softsub)',
      badge: '4Animo Softsub',
      url: `${embedBaseUrl}/ani/${anilistId}/${episodeNumber}/sub?${softsubParams}`,
    },
  ];
  ...
  if (malId && Number(malId) !== Number(anilistId)) {
    serversList.push({
      ...
      badge: 'MAL Mirror',
      url: `${embedBaseUrl}/hd-1/mal/${targetMalId}/${episodeNumber}/sub?${hardsubParams}`,
    });
  }
