...
export function getStreamData(animeId, episode = 1, animeDetails = null, season = 1, seasonEpisodes = []) {
  ...
  const isFinishedSeries =
    animeDetails?.status === 'FINISHED' ||
    animeDetails?.status === 'Ended' ||
    animeDetails?.status === 'COMPLETED' ||
    animeDetails?.status === 'CANCELED';
  ...
  if (isMovie) {
    ...
    episodesList = seasonEpisodes.map((ep) => {
      let isReleased = isFinishedSeries ? true : ep.isReleased !== false;
      if (!isReleased && ep.airDate) {
        const airTime = new Date(ep.airDate).getTime();
        if (!isNaN(airTime) && (airTime - 24 * 60 * 60 * 1000) <= Date.now()) {
          isReleased = true;
        }
      }
