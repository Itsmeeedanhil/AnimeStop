...
export function resolveStreamingServers(anilistId, episodeNumber = 1, malId = null, animeDetails = null) {
  ...
  const seasonNumber = tmdbInfo?.season || 1;
  const tmdbEpisodeNumber = (parseInt(episodeNumber, 10) || 1) + (tmdbInfo?.offset || 0);
  const targetMalId = malId || anilistId;

  const servers = [];

  // 1. Zoryva HD (Native MAL Engine - 100% accurate per-anime episode numbering & subtitles)
  servers.push({
    id: 'zoryva',
    name: 'Zoryva HD',
    url: `https://cdn.4animo.xyz/embed/hd-1/mal/${targetMalId}/${episodeNumber}/sub?k=1&clean=1&adblock=1&autoPlay=1`,
  });

  // 2. VidSrc Anime (Universal Native AniList stream)
  servers.push({
    id: 'vidsrc-ani',
    name: 'VidSrc Anime',
  ...
  // 3. TMDB Multi-Engine Mirrors (with accurate season & offset mapping)
  if (tmdbId) {
    if (isMovie) {
      servers.push(
        {
      ...
      servers.push(
        {
          id: 'vidsrc-to',
          name: 'VidSrc TO',
          url: `https://vidsrc.to/embed/tv/${tmdbId}/${seasonNumber}/${tmdbEpisodeNumber}`,
        },
        {
          id: 'vidsrc-me',
          name: 'VidSrc ME',
          url: `https://vidsrc.me/embed/tv?tmdb=${tmdbId}&season=${seasonNumber}&episode=${tmdbEpisodeNumber}`,
        },
        {
          id: 'vidlink',
          name: 'VidLink TMDB',
          url: `https://vidlink.pro/tv/${tmdbId}/${seasonNumber}/${tmdbEpisodeNumber}?primaryColor=ffe9b0&secondaryColor=121414`,
        },
        {
          id: '2embed',
          name: '2Embed TV',
          url: `https://www.2embed.cc/embedtv/${tmdbId}&s=${seasonNumber}&e=${tmdbEpisodeNumber}`,
        },
        {
          id: 'multiembed',
          name: 'MultiEmbed',
          url: `https://multiembed.mov/?video_id=${tmdbId}&tmdb=1&s=${seasonNumber}&e=${tmdbEpisodeNumber}`,
        }
      );
    }
  }
...
export function getStreamData(animeId, episode = 1, animeDetails = null) {
  ...
  const isAnons = statusStr.includes('ANONS') || statusStr === 'NOT_YET_RELEASED';

  if (isAnons) {
    releasedEpisodesCount = 0;
  } else if (statusStr === 'RELEASING') {
    if (episodesAired !== null && episodesAired !== undefined && !isNaN(episodesAired)) {
      ...
      releasedEpisodesCount = Math.min(episode, parseInt(totalEpisodes, 10));
    } else {
    ...
    releasedEpisodesCount = episodesAired !== null && !isNaN(episodesAired) ? episodesAired : parseInt(totalEpisodes || 12, 10);
  }

  const isCurrentEpisodeReleased = !isAnons && ((statusStr === 'FINISHED') || (releasedEpisodesCount > 0 && episode <= releasedEpisodesCount));
  const isUnreleased = isAnons || releasedEpisodesCount === 0 || !isCurrentEpisodeReleased;
