...
export async function searchAnime({
  ...
  // 1. Fallback: Live Kitsu Text Search
  if (q && q.trim()) {
    try {
      const res = await fetch(`https://kitsu.io/api/edge/anime?filter[text]=${encodeURIComponent(q.trim())}&page[limit]=20`, {
        headers: { 'Accept': 'application/vnd.api+json', 'User-Agent': 'AnimeStop/2.0' },
        signal: AbortSignal.timeout(4000),
      });
      if (res.ok) {
        ...
        if (Array.isArray(json.data) && json.data.length > 0) {
          const mapped = json.data.map(item => {
            ...
            return {
              ...
              idMal: attr.malId || parseInt(item.id, 10),
              title: { romaji, english },
              coverImage: { extraLarge: poster, large: poster, color: '#ffe9b0' },
          ...
          if (mapped.length > 0) {
            return {
              results: mapped,
              pageInfo: { total: mapped.length * 10, currentPage: 1, hasNextPage: false },
            };
          }
        }
      }
