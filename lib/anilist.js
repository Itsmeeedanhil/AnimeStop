...
export async function getAnimeDetails(id) {
  ...
  if (numericId) {
    try {
      ...
      // If no mapping, check if id is a direct Kitsu anime I
      if (!item) {
        try {
      ...
      if (item?.attributes) {
        ...
        for (const searchTitle of searchCandidates) {
          try {
            const query = `
        ...
        const romaji = attr.canonicalTitle || attr.titles?.en_jp || attr.titles?.en_kr || 'Anime';
        let english = attr.titles?.en || attr.titles?.en_us || null;
        if (!english && Array.isArray(attr.abbreviatedTitles)) {
          english = attr.abbreviatedTitles.find((t) =>
            typeof t === 'string' &&
            !/[\u0400-\u04FF\u3040-\u30ff\u3400-\u4dbf\u4e00-\u9fff\uf900-\ufaff\uff66-\uff9f\uac00-\ud7af]/.test(t)
          );
        }
        if (!english) english = attr.canonicalTitle || romaji;
        const native = attr.titles?.ja_jp || attr.titles?.ko_kr || attr.titles?.zh_cn || romaji;
