...
export async function getAnimeDetails(id, typeHint = null) {
  ...
  // 1. Try direct TMDB ID lookup first
  if (!isMovie) {
  ...
  const isJapanese = data && (data.original_language === 'ja' || !data.original_language || data.origin_country?.includes('JP'));
  const isValidDirectAnime = data && isAnimation && isJapanese;

  // 2. If direct TMDB ID didn't return a valid anime, check legacy AniList / MAL ID mapping
  if (!isValidDirectAnime) {
    const mapped = getTmdbFromIds(numericId);
    if (mapped?.tmdbId && mapped.tmdbId !== numericId) {
      let mappedIsMovie = mapped.type === 'movie';
      let mappedData = null;
      if (!mappedIsMovie) {
        mappedData = await fetchTmdb(`/tv/${mapped.tmdbId}`, {
          append_to_response: 'credits,recommendations,similar,videos,images',
        });
      }
      if (!mappedData) {
        mappedData = await fetchTmdb(`/movie/${mapped.tmdbId}`, {
          append_to_response: 'credits,recommendations,similar,videos,images',
        });
        if (mappedData) mappedIsMovie = true;
      }
      if (mappedData) {
        data = mappedData;
        isMovie = mappedIsMovie || isMovie;
      }
    }
  }

  // 3. If still not found or not animation/Japanese, fallback to AniList Title Lookup -> TMDB Search
  const currentIsAnimation = data && (data.genres || []).some((g) => g.id === 16 || g.name === 'Animation');
  const currentIsJapanese = data && (data.original_language === 'ja' || !data.original_language);

  if (!data || (!currentIsAnimation && !currentIsJapanese)) {
    try {
...
export async function getSeasonEpisodes(tvId, seasonNumber = 1) {
  ...
  // 1. Try direct TMDB TV season fetch first
  let data = await fetchTmdb(`/tv/${numericId}/season/${sNum}`);

  // 2. If direct fetch returned no episodes, check legacy AniList/MAL mapping
  if (!data || !data.episodes || data.episodes.length === 0) {
    const mapped = getTmdbFromIds(numericId);
    if (mapped?.tmdbId && mapped.tmdbId !== numericId) {
      data = await fetchTmdb(`/tv/${mapped.tmdbId}/season/${sNum}`);
    }
  }
  ...
  return data.episodes.map((ep) => {
    ...
    if (ep.air_date) {
      ...
      if (!isNaN(airTime)) {
        // 24h buffer for Japan Standard Time / global broadcast schedules
        isReleased = (airTime - 24 * 60 * 60 * 1000) <= Date.now();
      }
    }
