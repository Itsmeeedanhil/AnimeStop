...
const KNOWN_SEASON_AIRED_LIMITS = {
  // Bleach TYBW (TMDB 30984, Season 2): Parts 1-3 have aired 40 episodes. Part 4 (ep 41-50) is upcoming.
  '30984_2': 40,
};
...
export async function getSeasonEpisodes(tvId, seasonNumber = 1) {
  ...
  const limitKey = `${targetTmdbId}_${sNum}`;
  const maxAired = KNOWN_SEASON_AIRED_LIMITS[limitKey] ?? null;

  return data.episodes.map((ep) => {
    let isReleased = ep.air_date ? new Date(ep.air_date) <= new Date() : true;
    if (maxAired !== null && ep.episode_number > maxAired) {
      isReleased = false;
    }
